...
import math, random,time, CSP_Usage, CSP_With_Heuristic,Sudoku_Variable
...
def findNextUnassigned(board, n):
    for i in range(n):
        for j in range(n):
            if board[i][j].value == ".":
                return i, j
    return None, None
# main backtracking function without heuristics
def backTrackSudoku(board,n,widthOfSmallBoxes,numOfBacktracks=0):# i should use LCV here
    row, col=findNextUnassigned(board,n)
    if row is None:
        return True, numOfBacktracks
    for num in range(1,n+1):
        if isConsistent(board,row,col,num,n,widthOfSmallBoxes):
...
def runCSP(sodukoBoard, numberOfBoxes, widthOfSmallBoxes):
    return backTrackSudoku(sodukoBoard, numberOfBoxes,widthOfSmallBoxes)
# main part to run the program
if __name__ == "__main__":
    difficulity=""
    print("Welcome to the sodoku puzzle game, we wil try to solve using CSP with no heuristic!")
    while(True):
        try:
...
#Easy-> 50%->60% Filled
    ...
    solved, numberOfBacktracks=CSP_With_Heuristic.backTrackSudoku(skoduBoard,numberOfBoxes,widthOfSmallBoxes)
    if not solved:
